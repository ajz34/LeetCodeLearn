...
TEST(week177, e5172) {
    auto v = str_to_vec<int>("[8,1,9]");
    ASSERT_EQ(Solution().largestMultipleOfThree(v), "981");
    v = str_to_vec<int>("[8,6,7,1,0]");
    ASSERT_EQ(Solution().largestMultipleOfThree(v), "8760");
    v = str_to_vec<int>("[1]");
    ASSERT_EQ(Solution().largestMultipleOfThree(v), "");
    v = str_to_vec<int>("[0,0,0,0,0,0]");
    ASSERT_EQ(Solution().largestMultipleOfThree(v), "0");
}
...
>  ⚠️ **高计算复杂度提醒**
>
> 按下述思路解题的耗时尽管也不长 (16 ms)，但计算复杂度是 $O(n \log n)$，因为我们对数字进行了大小排序。
> 我看到另一种或许更简洁的解题方式是将数字存入长度为 10 的映射表中。这种做法的计算复杂度是 $O(n)$。内存复杂度大家应该都是 $O(n)$。
> 参考 [c++ 去掉最小值 8ms](https://leetcode-cn.com/problems/largest-multiple-of-three/solution/c-qu-diao-zui-xiao-zhi-8ms-by-yusenzhang_chatc/) 的题解。
...
PS. 这是我完成的第二道题。我上来就被前两道题卡死了 hh，特别是第一道题，我以前没用过 ctime，我在想 C++ 要拿头做呀……
